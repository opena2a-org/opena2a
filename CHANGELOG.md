...
- The bundled `hackmyagent` pin moves 0.25.2 to 0.30.0, exact. Both copies opena2a-cli resolved before this release were deprecated by us: `npm view hackmyagent@0.25.2 deprecated` prints a notice naming three defects, and `npm view hackmyagent@0.30.0 deprecated` is empty. This is the copy `scan`, `review`, `check`, `scan-soul`, `harden-soul` and `benchmark` resolve. **One behaviour change follows from it.** hackmyagent#373 (`check --json` exits 0 despite critical findings) is fixed from 0.26.1 on, and opena2a-cli returns the spawned scanner's exit code verbatim, so `opena2a check <target> --json` now exits non-zero where it previously exited 0. A pipeline that read exit 0 as "no critical findings" was reading a defect and will now fail where it used to pass. That is the fix working, but it is a CI-visible change and worth checking before you upgrade.
- `MIN_HMA_VERSION` moves 0.16.7 to 0.26.1. It governs a different copy from the pin above: the `hackmyagent` on your PATH, which `opena2a check` spawns and whose exit code it returns. Below 0.26.1 that binary carries #373, so the command cannot fail a CI job; opena2a-cli now says so on stderr once per process instead of staying silent. It warns and never blocks, because a below-floor scanner still produces a useful report — the defect is in the exit code, not the findings. The floor is deliberately not raised to match the pin, and deliberately does not exclude hackmyagent#406: that defect is in the `attack` verb, opena2a-cli exposes no `attack` command, and warning about a defect that cannot reach you through this tool would be noise.

### Known issues
- **A second, deprecated copy of `hackmyagent` survives this release, and one command still reaches it.** The pin above moves the copy opena2a-cli itself imports. It does not move the copy nested under `ai-trust`, which arrives through `ai-trust`'s own dependency range rather than ours: `opena2a-cli` declares `ai-trust: "^0.2.23"`, that caret resolves `ai-trust@0.2.25`, and `0.2.25` declares `hackmyagent: "^0.17.1"`. Measured in a clean install on 2026-08-11, that resolves `hackmyagent@0.17.11`, which is deprecated. **Verify in your own tree:** `npm ls hackmyagent` lists every resolved copy and its path, and `npm view hackmyagent@<version> deprecated` prints the notice for each. **What it affects:** only `opena2a registry <package>`, which delegates to `ai-trust check`, which runs the nested copy. `scan`, `review`, `check`, `scan-soul`, `harden-soul` and `benchmark` all use the pinned 0.30.0 and are unaffected. **Fix:** there is none you can apply from opena2a-cli, and we would rather say so than suggest one that does not work — an `overrides` block written here would be ignored, because npm does not honour `overrides` declared inside a published package. The fix has to come from an `ai-trust` release that admits a non-deprecated `hackmyagent`, followed by a bump of the `ai-trust` pin here; both are tracked. Until then, if you need a `registry` verdict from a current scanner, run `hackmyagent secure <target>` directly against the same target and compare.
