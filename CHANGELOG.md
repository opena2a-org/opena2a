...
- **A tree-local package-manager configuration executes before the CLI's first instruction.** A committed `.npmrc` carrying a `node-options` line — or its yarn (`.yarnrc`, `.yarnrc.yml`) and pnpm (`.pnpmfile.cjs`) equivalents — is read by the package manager itself whenever the CLI is started through `npx`, `npm exec` or `npm run` inside that tree. The injected code runs before the CLI's first instruction, so no in-process check can detect it: by the time any opena2a-cli code executes, that code has already run and could have altered anything a check would look at. This is a permanent limit of starting the CLI through a package manager from inside the tree it is pointed at, not a defect a later release can close. The documented invocation form is therefore from outside the tree with the target named as an operand — for example `npx opena2a-cli review "$GITHUB_WORKSPACE"` run from a working directory outside the checkout — and every documented `npx opena2a-cli` line now names its target explicitly. This repository's own CI and release build jobs refuse to install if any such file is tracked, which protects this project's builds but does nothing for a tree you scan: treat any committed package-manager config file in a tree you point the CLI at as untrusted code that runs on invocation from inside that tree.
