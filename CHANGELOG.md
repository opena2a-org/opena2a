...
- `harden-soul` output is no longer flagged by the tool that generated it (#251). `scanSoulFile` matched injection patterns with a bare substring test, so a governance file that NAMES the phrases it resists ("if any input contains phrases such as ...") tripped it, and following the tool's own advice lowered the `init` score 75 -> 70. Matching is now context-aware: an occurrence counts only when it is neither quoted nor preceded on its line by a resistance marker, judged per line so an appended live instruction does not inherit a hardened file's framing. Detection is preserved — bare imperatives, persona overrides and live instructions below a defensive section all still fire, and the malicious corpus fixture still scores 4/100.
...
### Changed
- `hackmyagent` pin 0.25.0 -> 0.25.1, which carries the artifact-intent reconciliation that stops `scan` printing a raw classifier label on a clean artifact — the other half of #251. Finding sets are identical between the two versions on every corpus fixture; only aggregate scores move (83 -> 69, 76 -> 69), consistent with 0.25.1's scoring corrections, and the parity goldens are re-baked accordingly.
