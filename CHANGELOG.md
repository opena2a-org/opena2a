...
### Security
- The `docker`, `python`, and `spawn` adapters now give delegated tools an allowlisted environment instead of the operator's entire environment (#228). Each previously handed `docker run`, `python3 -m <module>` and `npx <tool>` every variable the CLI was started with — cloud credentials, registry tokens, database URLs, model API keys — none of which is part of any adapter's contract. On one developer machine the measured difference is 128 variables reaching the child, 11 of them credentials, down to 20 and none. New `util/child-env.ts` builds the child environment from a base allowlist (PATH/HOME/locale, proxy and custom-CA settings, telemetry opt-out, the `*_CLI_PREFIX` citation vars) plus a per-adapter set (`DOCKER_*`, Python interpreter/virtualenv, npm/node toolchain), with a secret-name guard over prefix matches. Operators whose tooling needs a variable the allowlist omits can extend it with `OPENA2A_CHILD_ENV_ALLOW="NAME,PREFIX_*"`; a bare `*` is refused. The `release-smoke-comply` and `release-smoke-init` harnesses use the same allowlist. The `hackmyagent` delegation paths, `shield/llm-backend`, and the corpus parity harness still inherit the full environment and are tracked separately in #246 — each needs a per-variable decision, because HMA's contract includes credentials it genuinely uses and dropping them would disable capability silently rather than fail loudly.
- `shield recover --verify` now verifies before it recovers, which is what the flag has always claimed (#228). It previously called `exitLockdown()` first and re-entered lockdown only if the checks came back compromised, because `runIntegrityChecks` short-circuits while the lockdown marker is present. That briefly unlocked a compromised machine, and a check that threw — or a SIGINT, or a kill — during the window left it unlocked permanently. `runIntegrityChecks` takes a new `ignoreLockdown` option so the battery can run with the marker still in place; every other caller keeps the short-circuit.
