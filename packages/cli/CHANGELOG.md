...
This release ships work that merged to `main` after the `0.10.5` tag was cut. `0.10.5` published the bundled-hackmyagent `0.23.6` bump and the `scan-soul --explain` passthrough; the NanoMind classifier *adapter* landed in `0.10.5` but was not wired into the router (it shipped dormant). `0.10.6` is the release that actually delivers the trust-boundary gate to installed CLIs.
