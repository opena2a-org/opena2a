...
// Mock node:child_process so isClaudeCodeAvailable() returns false in tests.
// This ensures the API backend path is used (matching existing test expectations).
vi.mock('node:child_process', async (importOriginal) => {
  ...
  return {
    ...
    execFileSync: () => { throw new Error('not mocked'); },
  };
});
...
describe('checkLlmAvailable', () => {
  it('returns api backend when API key available and consented', async () => {
    process.env.ANTHROPIC_API_KEY = 'sk-test-key';
    _llmEnabled = true;
    const result = await checkLlmAvailable();
    expect(result.backend).toBe('api');
    expect(result.apiKey).toBe('sk-test-key');
  });

  it('returns none backend when API key is missing', async () => {
    delete process.env.ANTHROPIC_API_KEY;
    const result = await checkLlmAvailable();
    expect(result.backend).toBe('none');
  });

  it('returns none backend when LLM is disabled', async () => {
    process.env.ANTHROPIC_API_KEY = 'sk-test-key';
    _llmEnabled = false;
    const result = await checkLlmAvailable();
    expect(result.backend).toBe('none');
