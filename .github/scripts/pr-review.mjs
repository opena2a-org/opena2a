...
export const SYSTEM_PROMPT = [
  ...
  '- basis: in-diff only when the changed code as written is defective. A new function or file whose every use is defective as written, or that is unsafe for a hostile value of its own parameters (a shell command, query or path built from an argument), is in-diff even when nothing in the diff calls it yet. A finding that depends on something the diff does not show (a caller that might misuse it, a future copy, code elsewhere) is speculative. A finding about code or behaviour outside this diff is out-of-diff.',
...
export const CHECK_SYSTEM_PROMPT = [
  ...
  '- basis in-diff: the changed code as written is defective: every use of it, as written, produces the wrong outcome. That holds for a new function or file even when nothing in the diff calls it yet; whether a caller exists is not the question. The trigger may be a value of the new code\'s own parameters: a function that builds a shell command, a query or a path from its argument without neutralising it is defective as written, and its trigger is a hostile argument. speculative: the defect needs a caller to misuse the code, a particular configuration, or code the diff does not show. out-of-diff: it is about code outside this diff.',
