...
Each block writes three files into the current directory: the tarball, the registry's list of attestations for that version, and the SLSA v1 provenance bundle taken from that list. It runs in a subshell, so a failed step ends the block without closing the terminal. It prints the package and version it checks, then the result from cosign. The check passed only if the last line is `Verified OK`. Notices from npm, such as an available update, can also appear. They are not part of the result.
...
(
  ...
  [[ $v =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || { echo "version check failed: expected a release version of hackmyagent, three numbers separated by dots" >&2; exit 1; }
  npm pack "hackmyagent@$v" --loglevel=error >/dev/null
  ...
  curl -sSf "https://registry.npmjs.org/-/npm/v1/attestations/hackmyagent@$v" > hackmyagent.attestations.json
  jq -e '.attestations[] | select(.predicateType=="https://slsa.dev/provenance/v1") | .bundle' hackmyagent.attestations.json > hackmyagent.sigstore.json || { echo "attestation check failed: expected SLSA v1 provenance for hackmyagent@$v" >&2; exit 1; }
  echo "hackmyagent@$v"
  cosign verify-blob-attestation --bundle hackmyagent.sigstore.json --type slsaprovenance1 --certificate-identity "https://github.com/opena2a-org/hackmyagent/.github/workflows/release.yml@refs/tags/v$v" --certificate-oidc-issuer https://token.actions.githubusercontent.com --digestAlg sha512 --digest "$(openssl dgst -sha512 -r "hackmyagent-$v.tgz" | cut -d' ' -f1)"
)
```
...
(
  ...
  [[ $v =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || { echo "version check failed: expected a release version of secretless-ai, three numbers separated by dots" >&2; exit 1; }
  npm pack "secretless-ai@$v" --loglevel=error >/dev/null
  ...
  curl -sSf "https://registry.npmjs.org/-/npm/v1/attestations/secretless-ai@$v" > secretless-ai.attestations.json
  jq -e '.attestations[] | select(.predicateType=="https://slsa.dev/provenance/v1") | .bundle' secretless-ai.attestations.json > secretless-ai.sigstore.json || { echo "attestation check failed: expected SLSA v1 provenance for secretless-ai@$v" >&2; exit 1; }
  echo "secretless-ai@$v"
  cosign verify-blob-attestation --bundle secretless-ai.sigstore.json --type slsaprovenance1 --certificate-identity "https://github.com/opena2a-org/secretless-ai/.github/workflows/release.yml@refs/tags/v$v" --certificate-oidc-issuer https://token.actions.githubusercontent.com --digestAlg sha512 --digest "$(openssl dgst -sha512 -r "secretless-ai-$v.tgz" | cut -d' ' -f1)"
)
```
...
(
  ...
  [[ $v =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || { echo "version check failed: expected a release version of opena2a-cli, three numbers separated by dots" >&2; exit 1; }
  npm pack "opena2a-cli@$v" --loglevel=error >/dev/null
  ...
  curl -sSf "https://registry.npmjs.org/-/npm/v1/attestations/opena2a-cli@$v" > opena2a-cli.attestations.json
  jq -e '.attestations[] | select(.predicateType=="https://slsa.dev/provenance/v1") | .bundle' opena2a-cli.attestations.json > opena2a-cli.sigstore.json || { echo "attestation check failed: expected SLSA v1 provenance for opena2a-cli@$v" >&2; exit 1; }
  echo "opena2a-cli@$v"
  cosign verify-blob-attestation --bundle opena2a-cli.sigstore.json --type slsaprovenance1 --certificate-identity-regexp "^https://github\.com/opena2a-org/opena2a/\.github/workflows/release\.yml@refs/tags/(cli-)?v${v//./\\.}\$" --certificate-oidc-issuer https://token.actions.githubusercontent.com --digestAlg sha512 --digest "$(openssl dgst -sha512 -r "opena2a-cli-$v.tgz" | cut -d' ' -f1)"
)
```
...
(
  ...
  [[ $v =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] || { echo "version check failed: expected a release version of @opena2a/aim-core, three numbers separated by dots" >&2; exit 1; }
  npm pack "@opena2a/aim-core@$v" --loglevel=error >/dev/null
  ...
  curl -sSf "https://registry.npmjs.org/-/npm/v1/attestations/@opena2a%2faim-core@$v" > opena2a-aim-core.attestations.json
  jq -e '.attestations[] | select(.predicateType=="https://slsa.dev/provenance/v1") | .bundle' opena2a-aim-core.attestations.json > opena2a-aim-core.sigstore.json || { echo "attestation check failed: expected SLSA v1 provenance for @opena2a/aim-core@$v" >&2; exit 1; }
  echo "@opena2a/aim-core@$v"
  cosign verify-blob-attestation --bundle opena2a-aim-core.sigstore.json --type slsaprovenance1 --certificate-identity "https://github.com/opena2a-org/opena2a/.github/workflows/release.yml@refs/tags/aim-core-v$v" --certificate-oidc-issuer https://token.actions.githubusercontent.com --digestAlg sha512 --digest "$(openssl dgst -sha512 -r "opena2a-aim-core-$v.tgz" | cut -d' ' -f1)"
)
```
...
| `version check failed: expected a release version` | The version, from `npm view` or from the version variable, is not three numbers separated by dots. | If you set the variable, correct it. Otherwise, report it. To see the value safely, run the block's `npm view` command with `--json`, for example `npm view hackmyagent version --json`, which prints the value in quotes with control characters, including line breaks, escaped. |
| `code ETARGET` | The registry lists no such version of the package. | If you set the variable, correct it. Otherwise, report it. |
| `code EINTEGRITY` | The tarball npm fetched does not match the checksum the registry records for that version. | Report it. |
| `code ENOTFOUND`, `code ECONNREFUSED` or another network code from npm | npm could not reach the registry, for example because the network or a proxy is down. | Check the network and npm's proxy and registry settings, then run the block again. |
| Any other `code` from npm | npm stopped before the block could check the tarball. | Fix the problem npm describes, then run the block again. If it fails the same way again, report it. |
...
| `attestation check failed: expected SLSA v1 provenance for` and the package and version | The registry's list of attestations for that version holds no SLSA v1 provenance, or is not valid JSON. | Report it. |
...
| Anything else, including no output | The block did not finish, or it failed in a way this table does not list. | Check the prerequisites and anything the output names, then run the block again. If it fails again, report it. |

To report, follow the security policy of the repository that publishes the package: [hackmyagent](https://github.com/opena2a-org/hackmyagent/blob/main/SECURITY.md), [secretless-ai](https://github.com/opena2a-org/secretless-ai/blob/main/SECURITY.md), or [opena2a](https://github.com/opena2a-org/opena2a/blob/main/SECURITY.md) for `opena2a-cli` and `@opena2a/aim-core`. Include the package, the version and the block's full output, and keep the files the block wrote.

The messages above were copied from runs of these blocks with npm 10.9.3 and cosign 3.1.3. Captures of each run are in [`docs/captures/`](https://github.com/opena2a-org/opena2a/tree/main/docs/captures).
